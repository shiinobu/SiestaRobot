...
from NaoRobot.modules.helper_funcs.extraction import extract_user_and_text
from NaoRobot.modules.helper_funcs.string_handling import extract_time
from NaoRobot.modules.log_channel import gloggable, loggable
...
def unbanb_btn(update: Update, context: CallbackContext) -> str:
    ...
    if query.data != "unbanb_del":
        ...
        if query_match == "unbanb_unban":
            ...
            query.message.edit_text(f"Yep, {mention_html(member.user.first_name)} can join!")
...
def unban(update: Update, context: CallbackContext) -> str:
    ...
    chat.unban_member(user_id)
    message.reply_text(f"Yep, {mention_html(member.user.first_name)} can join!")
...
def selfunban(update: Update, context: CallbackContext) -> str:
    ...
    message.reply_text(f"Yep, I have unbanned {mention_html(member.user.first_name)}.")
